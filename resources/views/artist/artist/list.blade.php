            ...
            <div class="card">
                <div class="card-body">
                    <div class="float-right">
                        ...
                        @endcan
                        @can('artist_create')
                            <a class="btn btn-v-green" href="{{ route('artist.'.request()->segment(1).'.create') }}">{{ trans('global.add') }} Artist</a>
                        @endcan
                    </div>
                ...
                <div class="modal-body">
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover table-striped">
                            ...
                            <tr>
                                <th>{{ucwords(str_replace('_',' ','spotify'))}}</th>
                                <td id="spotify" align="center"></td>
                            </tr>
                            <tr>
                                <th>{{ucwords(str_replace('_',' ','soundcloud'))}}</th>
                                <td id="soundcloud" align="center"></td>
                            </tr>
                            <tr>
                                <th>{{ucwords(str_replace('_',' ','applemusic'))}}</th>
                                <td id="applemusic" align="center"></td>
                            </tr>
                            <tr>
...
@section('script-bottom')
    <script>
        $(function(){
            var source = `{{ route('artist.'.request()->segment(1).'.index') }}`;
            ...
            $(document, this).on('click', '.view', function () {
                let id = $(this).attr('id');
                $.ajax({
                    url: `{{url(request()->segment(1).'/show/')}}/${id}`,
                    dataType: "json",
                    success: function (data) {
                        ...
                        document.getElementById('spotify').innerText = data.spotify;
                        document.getElementById('soundcloud').innerText = data.soundcloud;
                        document.getElementById('applemusic').innerText = data.applemusic;
            ...
            $(document).on('click', '#ok_delete', function () {
                $.ajax({
                    type: "delete",
                    url: `{{url(request()->segment(1).'/'.request()->segment(2).'/destroy/')}}/${delete_id}`,
                    headers: {
            ...
            document.getElementById('delete_all').addEventListener('click', () => {
                ...
                if (checkboxes.length > 0) {
                    ...
                    let ajax = async () => {
                        await fetch(`{{route('artist.'.request()->segment(1).'.delete_all')}}`, {
                            method: "post",
                            headers: {
